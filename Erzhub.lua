local Players =-- Erz Hub | discord.gg/tZ9BvMxcD | Nicht leaken / weitergeben!
if _G.ErzHubLoaded then return end
_G.ErzHubLoaded = true

print("Erz Hub erfolgreich geladen! | discord.gg/tZ9BvMxcD")

-- dein normaler Code beginnt hier ↓↓↓
local Players = game:GetService("Players")
-- ... rest des Scripts ... game:GetService("Players")
...
local function createBillboard(prompt)
	if not prompt or billboardCache[prompt] then return end
	...
	local function update()
		...
		if string.find(obj, "disallow") then
			textLabel.Text = "✅ ALLOWED"
			textLabel.TextColor3 = Color3.fromRGB(50, 255, 50)
		else
			textLabel.Text = "🔒 DISALLOWED"
	...
	local bob = RunService.RenderStepped:Connect(function()
		...
		billboard.StudsOffsetWorldSpace = Vector3.new(0, 3.5 + math.sin(tick() * 2) * 0.5, 0)
	end)
	billboardCache[prompt] = {billboard = billboard, conns = {bob, prompt:GetPropertyChangedSignal("ObjectText"):Connect(update)}}
end

local function shouldMonitor(prompt)
	local text = (prompt.ObjectText .. prompt.ActionText):lower()
	return string.find(text, "friend")
end

local function enableFriendESP()
	if descendantAddedConn then return end
	descendantAddedConn = game:GetService("Workspace").DescendantAdded:Connect(function(d)
		if d:IsA("ProximityPrompt") then
			task.wait(0.1)
			if shouldMonitor(d) then
				createBillboard(d)
			end
		end
	end)
	for _, d in ipairs(game:GetService("Workspace"):GetDescendants()) do
		if d:IsA("ProximityPrompt") and shouldMonitor(d) then
			createBillboard(d)
		end
	end
end

local function disableFriendESP()
	if descendantAddedConn then
		descendantAddedConn:Disconnect()
		descendantAddedConn = nil
	end
	for _, cache in pairs(billboardCache) do
		for _, conn in ipairs(cache.conns) do
			conn:Disconnect()
		end
...
if CoreGui:FindFirstChild("ErzHubGui") then
	CoreGui["ErzGui"]:Destroy()
end
local screenGui = Instance.new("ScreenGui")
screenGui.Name = "ErzGui"
...
createESPBox(standing1, "Standing 2")
createESPBox(standing2, "Standing 1")
...
uiGradient.Color = ColorSequence.new({
	...
	ColorSequenceKeypoint.new(0.5, Color3.fromRGB(30, 0, 255)),
...
createToggle("Base Protector", UDim2.new(0.05, 0, 0, 90), function(state)
	_G.AutoPotion = state
end)
...
local function updateVelocity()
	if player:GetAttribute("Stealing") then
		...
		local target = Vector3.new(moveDir.X * currentSpeed, hrp.Velocity.Y, moveDir.Z * currentSpeed)
		local curr = hrp.Velocity
		hrp.Velocity = curr:Lerp(target, 0.45)
	end
end
local function enableBooster()
	if speedConnection then return end
	speedConnection = RunService.Heartbeat:Connect(updateVelocity)
end
...
createToggle("Friend ESP", UDim2.new(0.05, 0, 0, 160), function(state)
	friendESPEnabled = state
	if state then
		enableFriendESP()
	else
		disableFriendESP()
...
dropdownContainer.Position = UDim2.new(0.05, 0, 0, 195)
...
dropdownButton.Font = Enum.Font.MontserratBold
...
discordText.Text = "discord.gg/tSBkpD66"
discordText.TextColor3 = Color3.fromRGB(150, 150, 150)
discordText.TextSize = 8
discordText.Font = Enum.Font.MontserratMedium
...
local notificationGui = plrGUI:WaitForChild("Notification").Notification
...
notificationGui.ChildAdded:Connect(function(child)
	if child:IsA("TextLabel") and _G.AutoPotion then
		if string.find(child.Text, "Someone is stealing") then
			getPlayer()
			local message = cmd .. targetPlayer.Name
			chatService:WaitForChild("TextChannels").RBXGeneral:SendAsync(message)
		end
	end
end)
...
antiStealButton.Position = UDim2.new(0.05, 0, 0, 230)
antiStealButton.Text = "Auto tp Left"
antiStealButton.Font = Enum.Font.MontserratBold
...
autoTpRightButton.Position = UDim2.new(0.05, 0, 0, 265)
autoTpRightButton.Text = "Auto tp Right"
autoTpRightButton.Font = Enum.Font.MontserratBold
...
bar.Position = UDim2.new(0.05, 0, 0, 300)
...
dropdownContainer.Name = "DropdownContainer"
dropdownContainer.Size = UDim2.new(0.9, 0, 0, 30)
dropdownContainer.Position = UDim2.new(0.05, 0, 0, 325)
dropdownContainer.BackgroundTransparency = 1
dropdownContainer.ClipsDescendants = true
dropdownContainer.Parent = mainFrame
local dropdownButton = Instance.new("TextButton")
...
mainFrame.Size = UDim2.new(0, 240, 0, 415)
